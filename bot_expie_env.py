...
async def cmd_generate(ctx, *, prompt=None):
    """!ген2 <описание> — сгенерировать картинку через AI Horde."""
    
    if not prompt:
        prompt = "solo, cute, fluffy, black melanistic fur, anthro, furry, wolf-fox hybrid, big eyes, orange sclera, big fluffy tail, orange tip tail, three ears, high quality, kawaii style, beautiful background"
    
    # Улучшаем промпт
    enhanced_prompt = f"{prompt}, furry art, detailed fur, soft lighting, high quality"
    
    async with ctx.typing():
        try:
            headers = {
                "Content-Type": "application/json",
                "apikey": "0005842000",  # Анонимный ключ — можно любой
                "Client-Agent": "ExpieDiscordBot:1.0:(by Anonymous)"
            }
            ...
            payload = {
                ...
                "models": ["SDXL 1.0"],  # или "FLUX.1 [schnell]"
                "nsfw": False,
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status != 202:
                        ...
                        await ctx.reply(f"*прижимает уши* Horde не принял заказ: {resp.status}")
                        return
                    
                    data = await resp.json()
                    job_id = data["id"]
                    
                    await ctx.reply("*виляет хвостом* Заказал картинку! Рисуют... Это может занять минуту ⏳")
                    
                    # 2. Ждём готовности (проверяем каждые 3 секунды)
                    image_url = None
                    for _ in range(40):  # Максимум 2 минуты ожидания
                    ...
                    if not image_url:
                        await ctx.reply("*прижимает уши* Художники заняты... Попробуй позже!")
                        return
                    ...
                    async with session.get(image_url) as img_resp:
                        ...
                        file = discord.File(
                            fp=io.BytesIO(image_data),
                            filename="expie_horde.png"
                        )
                        
                        await ctx.reply(
                            content=f"*виляет хвостом* Готово! По запросу: `{prompt[:300]}`",
                            file=file
                        )
