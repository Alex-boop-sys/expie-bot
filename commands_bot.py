...
def register_commands(bot):
    ...
    async def cmd_hug(ctx, member: discord.Member = None):
        ...
        phrases = [
            f"*тихо подкрадывается сзади и обнимает **{member.display_name}** лапками за талию*\n"
            f"Ты такой тёплый... *трётся мордочкой о спину* И такой мягкий. "
            f"*прижимает уши от счастья* Можно я тут побуду? 🐾",

            f"*подбегает на задних лапах и с разбега прыгает к **{member.display_name}** на колени.*\n"
            f"Оп! *устраивается поудобнее, сворачиваясь калачиком* Тут так тепло и прияно... "
            f"*мурчит как котёнок* Теперь я буду сидеть здесь. Лучшее место! 🧡",

            f"*залезает на плечи к **{member.display_name}** и садится свесив лапки.*\n"
            f"Смотри, я выше всех! *шевелит хвостом, обнимая шею оранжевым кончиком* "
            f"Твои плечи удобные... Можно тут посидеть? *фыркает в ухо* 🦊",

            f"*подползает ближе и тихо кладёт голову на колени к **{member.display_name}***\n"
            f"*смотрит оранжевыми глазами снизу вверх* Ты хороший... "
            f"*шевелит ушками, то которое сзади тоже поворачивается* "
            f"Не уходи пока, ладно? Я тут присмотрю за тобой. *мягко обнимает лапками* 🧡",
        ]
